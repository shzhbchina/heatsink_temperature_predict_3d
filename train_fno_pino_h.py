...
'''
python /workspace/src/train_fno_pino_h.py \
...
class HeatsinkH5:
    def __init__(self, path):
        ...
        self.h_truth = None  # (Nt,Nz,Ny,Nx) 或 None
        # 优先路径：compute_h_from_h5.py 输出
...
class PairDataset(torch.utils.data.Dataset):
    def __init__(self, H: HeatsinkH5):
        ...
        # 真值 h（如存在）
        self.h_truth = torch.from_numpy(H.h_truth) if (H.h_truth is not None) else None  # (Nt, Z,Y,X)
    ...
    def __getitem__(self, idx):
        ...
        # 如果有真值 h：提供当前目标时刻 t1 的场（接口上非零）
        if self.h_truth is not None:
            cond["h_truth_t0"] = self.h_truth[t0][None, ...]  # (1,Nz,Ny,Nx)
...
def pde_residual_fv(theta_next, theta_curr, dtau, Ms, Mi, Bi_eff, dxh_dyh_dzh, S):
    dz, dy, dx = _unpack_deltas(dxh_dyh_dzh)
    Div = fv_divergence(theta_next, Ms, Mi, Bi_eff, dz, dy, dx, mode='interior')
...
def train(args):
    ...
    for ep in range(start_ep, args.epochs+1):
        ...
        for x, y, cond in dl:
            ...
            # 若有真值 h：取 t1 的 (B,1,Z,Y,X)
            h_truth_t0 = cond.get("h_truth_t0", None)
            if h_truth_t0 is not None:
                h_truth_t0 = h_truth_t0.to(device).float().repeat(x.size(0),1,1,1,1)  # (B,1,...)
            # 预测 r -> y_pred
            r = model(x)
            ...
            # ----- 计算 Bi_eff：优先用真值 h ；否则用小网预测；再否则用 Bi_base -----
            ...
            if (h_truth_t0 is not None) and args.use_h_truth_for_bc:
                # 用真值 h 替换 BC（Mi 外部为 0，无影响）
                Bi_eff = (h_truth_t0 * L_over_k)
                # 如果还在学 h：用真值监督小网络
                if (hhead is not None) and (args.lam_h_sup > 0.0):
                    ...
                    Tinf_abs_norm = torch.tensor(Tinf_abs_norm).to(device).float().repeat(x.size(0),1,x.size(2),x.size(3),x.size(4))  # (B,1,...)
                    feats = torch.cat([
                        x[:, 0:1],  # θ0
                        x[:, 2:3],  # S
                        ...
                        nsurf[:, 0:1], nsurf[:, 1:2], nsurf[:, 2:3],
                        dtau_field,
                        Tinf_abs_norm  # 绝对环境温度（归一化）
                    ], dim=1)  # => in_c = 10

                    # delta_h_raw = hhead(feats) * Mi
                    delta_h_raw = hhead(feats)
                    # 你的头目前就是直接 softplus(delta_h) 当作 h_pred（接口上）；体内 0
                    #h_pred = F.softplus(delta_h_raw) * Mi
                    h_pred = delta_h_raw
                    h_sup_loss = ((h_pred - h_truth_t0)**2 * Mi).sum() / (Mi.sum() + 1e-8)
            else:
                # 没有真值 h（或不启用）：走原逻辑
                if hhead is not None:
                    # feats = torch.cat([
                    ...
                    Tinf_abs_norm = (H.T_amb - 300.0) / 50.0  # (1,Z,Y,X)
                    Tinf_abs_norm = torch.tensor(Tinf_abs_norm).to(device).float().repeat(x.size(0), 1, x.size(2),
                                                                                          x.size(3),
                                                                                          x.size(4))  # (B,1,...)
                    feats = torch.cat([
                        x[:, 0:1],  # θ0
            ...
            # BC 残差
            if args.use_bc:
                # theta_inf = (cond["Tinf"].to(device).float() - H.T_amb) / H.dTref
                # theta_inf = theta_inf.repeat(x.size(0), 1, 1, 1, 1)
                Tinf_abs_norm = (H.T_amb -  H.T_amb) / cond['dtau']  # (1,Z,Y,X)
                Tinf_abs_norm = Tinf_abs_norm.to(device).float().repeat(x.size(0), 1, x.size(2),
                                                                                      x.size(3), x.size(4))  # (B,1,...)

                res_b = bc_robin_residual(y_pred, Bi_eff, nsurf, dxh_dyh_dzh, Ms=Ms, Mi=Mi,theta_inf=Tinf_abs_norm )
