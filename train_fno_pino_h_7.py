...
class PairDataset(torch.utils.data.Dataset):
    def __init__(self, H: HeatsinkH5, max_skip: int = 1):  # NEW: max_skip
        self.H = H
        self.Nt = H.T.shape[0]
        self.starts = list(range(self.Nt - 1))              # NEW: 以起点索引为样本
        self.max_skip = int(max(1, max_skip))               # NEW
    ...
    def __getitem__(self, idx):
        t0 = self.starts[idx]
        # NEW: 随机跨度 k ∈ [1, max_skip]，确保不越界
        k_max = min(self.max_skip, self.Nt - 1 - t0)
...
class HHead(nn.Module):
    def __init__(self, in_c=7, width=32, layers=2, out_feat=64, mlp_hidden=64,
                 ...
                 h_prior=10.0, beta=2.0, use_ctx_film=True, ctx_dim=10):  # CHANGED: ctx_dim=10
        """
...
def train(args):
    ...
    model = FNO3D(in_c=3, width=args.width, modes=(args.mz, args.my, args.mx), layers=args.layers,
                  context_dim=10).to(device)             # CHANGED
    ...
    if args.learn_h:
        ...
        hhead = HHead(in_c=7, width=args.h_width, layers=args.h_layers, mask_idx=2,
                      ...
                      use_ctx_film=True, ctx_dim=10).to(device)  # CHANGED
    ...
    for ep in range(start_ep, args.epochs + 1):
        ...
        for x, y, cond in dl:
            ...
            # 参数：L/k 与 全局 context（含 log(alpha)） + NEW: log(dtau)
            L_over_k = cond["L_over_k"].to(device).view(1, 1, 1, 1, 1)
            ctx_glb = cond["ctx_glb"].to(device).repeat(x.size(0), 1)  # (B,9)
            dtau_ctx = torch.log(dtau.view(-1, 1) + 1e-12)             # NEW: (B,1)
            ctx_ext = torch.cat([ctx_glb, dtau_ctx], dim=1)            # NEW: (B,10)
            ...
            if args.rk2:
                # k1
                r1 = model(x, ctx_ext)
                theta_tilde = x[:, 0:1] + dtau.view(-1, 1, 1, 1, 1) * r1
                x2 = torch.cat([theta_tilde, x[:, 1:2], x[:, 2:3]], dim=1)
                # k2
                r2 = model(x2, ctx_ext)
                r = 0.5 * (r1 + r2)
            else:
            ...
            if (hhead is not None) and ("h_truth_t0" in cond) and (cond["h_truth_t0"] is not None) and (args.lam_h_sup > 0.0):
                ...
                h_pred_field = hhead(feats, ctx_vec=ctx_ext)      # CHANGED: ctx_ext (10维)
