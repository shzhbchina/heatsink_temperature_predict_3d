...
'''
python /workspace/src/train_fno_pino_h_3.py \
  ...
  --learn_h --h_width 16 --h_layers 2 --lam_h_l2 0.0 \
...
class HeatsinkH5:
    def __init__(self, path):
        ...
        origin  = gp['origin'][:] if 'origin' in gp else np.array([0.0,0.0,0.0], dtype=np.float64)
        ...
        # 重力（若无则用 (0,-9.81,0)），按 [gz,gy,gx] 与 z,y,x 对齐
        if 'g' in cst:
        ...
        self.L      = float(sc['L'][()])     # 旧 L：用于 θ 的无量纲化
...
class PairDataset(torch.utils.data.Dataset):
    def __init__(self, H: HeatsinkH5):
        ...
        self.dxh, self.dyh, self.dzh = H.dz_hat, H.dy_hat, H.dx_hat
        self.h_truth = torch.from_numpy(H.h_truth) if (H.h_truth is not None) else None

        # 外部参数（不进 voxel 通道）
        self.ctx_vec = torch.from_numpy(H.ctx_vec)        # (7,)
        self.Lchar_over_k = torch.tensor([H.Lchar_over_k], dtype=torch.float32)  # (1,)
...
class FNN3D(nn.Module):
    def __init__(self, in_c=3, width=64, layers=6, add_coords=True, fourier_k=4, dropout=0.0):
        super().__init__()
...
# ------------------------- HHead：标量 h × mask（支持外部“上下文参数”FiLM） -------------------------
...
def train(args):
    ...
    hhead = None
    if args.learn_h:
        # HHead 输入通道 = 8：[θ0, S, Mi, Ms, nz,ny,nx, theta_inf]
        hhead = HHead(in_c=8, width=args.h_width, layers=args.h_layers, mask_idx=2,
                      h_min=0.0, h_max=30.0, h_prior=10.0, beta=2.0,
                      use_ctx_film=True, ctx_dim=7).to(device)
    ...
    if args.out_dir: out_dir = args.out_dir
        ...
        if args.resume and args.ckpt: out_dir = os.path.dirname(os.path.abspath(args.ckpt))
        else:
    ...
    print(f"[info] g={H.g_vec} (|g|={H.g_mag:.3f}), L_char={H.L_char:.4f} m, Lchar/k={H.Lchar_over_k:.4e} K/W·m^-1")
    print(f"[info] scales: alpha={H.alpha:.3e}, L_ref={H.L:.3e}, dTref={H.dTref:.3e}, qref={H.qref:.3e}")
    ...
    for ep in range(start_ep, args.epochs+1):
        ...
        for x, y, cond in dl:
            ...
            # 关键：用 L_char/k
            Lchar_over_k = cond["Lchar_over_k"].to(device).view(1,1,1,1,1)

            # 外部上下文（不作为通道）—— 修正维度：先 unsqueeze 再 repeat
            ctx_vec = cond["ctx_vec"].to(device).repeat(x.size(0),1)  # (B,7)
            ...
            # ====== 构造“数据驱动”的逐体素 h，用于 BC/PDE（完全不依赖 hhead）======
            if "h_truth_t0" in cond and cond["h_truth_t0"] is not None:
                h_data = cond["h_truth_t0"].to(device).float().repeat(Bsz,1,1,1,1)   # (B,1,Z,Y,X)
            else:
                h_data = cond["h_init"].to(device).float().repeat(Bsz,1,1,1,1)       # (B,1,Z,Y,X)

            # 仅在界面上有效（可选）：把非界面体素清零，数值上更稳健
            h_data = h_data * Mi

            Bi_eff = h_data * Lchar_over_k   # Bi = h * L_char / k
...
def parse_args():
    ...
    ap.add_argument("--lam_h_l2", type=float, default=0.0)
    ap.add_argument("--lam_h_sup", type=float, default=1.0, help="真值 h 对小网络 h 的监督权重 (MSE)")
    ap.add_argument("--use_h_truth_for_bc", action="store_true", default=True, help="（保留但不再决定 Bi 来源）")
