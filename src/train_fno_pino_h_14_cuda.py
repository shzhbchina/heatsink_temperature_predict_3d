...
从头：
python src/train_fno_pino_h_14_cuda.py \
...
python src/train_fno_pino_h_14_cuda.py \
  --data_glob "dat_test/*.h5" \
  --epochs 400 --batch 1 --bucket-by-shape --lr 1e-3 \
  --width 24 --layers 8 --mz 12 --my 24 --mx 4 \
  --rk2 \
  --use_pde --lam_pde 100.0 --use_bc --lam_bc 0.0 --lam_sup 1.0 \
  ...
  --resume \
  --device cuda:0 --twoframe \
  --ckpt_every 1

#两连续帧(使用--rk2可用可不用）
python src/train_fno_pino_h_14_cuda.py \
  --data_glob "dat_test/*.h5" \
  --epochs 400 --batch 1 --bucket-by-shape --lr 2e-4 \
  --width 24 --layers 8 --mz 12 --my 24 --mx 4 \
  --use_pde --lam_pde 100.0 --use_bc --lam_bc 0.0 --lam_sup 1.0 \
  --use_energy --lam_energy 2.0 \
  --air-anchor 1e-1 \
  --multi-steps 1 --max_skip 1 \
  --spec-pad-type reflect --spec-pad-z 8 --spec-pad-y 8 --spec-pad-x 8 \
  --out_dir "model_param/heatsink_fno_pino_learnh_8ls_temp" \
  --ckpt "model_param/heatsink_fno_pino_learnh_8ls/ckpt_ep46.pt" \
  --resume \
  --device cuda:0\
  --ckpt_every 1\
  --rk2
