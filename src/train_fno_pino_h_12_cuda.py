...
要点：
1) UFNO 多分辨率修复：每层内含 n_scales=3 个 SpectralConv3d 分支，modes 按 1/2/4 缩减，
   并用 softmax(scale_logits) 混合输出；键名为：
   specs.<layer>.specs.<scale>.(weight|gate) 以及 specs.<layer>.scale_logits
   ——与 v11 ckpt 完全对齐，可无缝加载。
2) 仍保留：(A) 逐层 FiLM、(C) 频域门控 gate、FFT 前 reflect/replicate padding。
3) 训练支持：Δθ 监督、FV/Strong PDE 残差、Robin BC、能量约束、RK2 时间推进。
4) 多步推进（可选）：--multi-steps N；若数据不含中间真值，则仅最后一步做监督，
   PDE/BC/能量在每步参与；支持 --detach-every k 控制显存。

示例（从头训）：
python src/train_fno_pino_h_12_cuda.py \
...
示例（续训，允许多步+RK2；若 ckpt 源自 v11，多分辨率权重将正确对齐）：
python src/train_fno_pino_h_12_cuda.py \
  --data_glob "dat_test/*.h5" \
  --epochs 400 --batch 1 --lr 1e-3 \
  --width 24 --layers 4 --mz 12 --my 24 --mx 4 \
  --multi-steps 2 --max_skip 2 --rk2 --detach-every 0 \
  --use_pde --lam_pde 1.0 --use_bc --lam_bc 0.0 --lam_sup 1.0 \
  --use_energy --lam_energy 1.0 \
  --air-anchor 1e-1 \
  --spec-pad-type reflect --spec-pad-z 8 --spec-pad-y 8 --spec-pad-x 8 \
  --out_dir "model_param/heatsink_fno_pino_h" \
  --ckpt "model_param/heatsink_fno_pino_learnh/ckpt_ep70.pt" \
  --device cuda:0 --num-workers 0 --pin-memory --preload-gpu \
  --ckpt_every 1 \
  --resume
续训（worker）
python src/train_fno_pino_h_12_cuda.py \
  --data_glob "dat_test/*.h5" \
  --epochs 400 --batch 1 --bucket-by-shape --lr 1e-3 \
  --width 24 --layers 4 --mz 12 --my 24 --mx 4 \
  --multi-steps 2 --max_skip 1 --rk2 --detach-every 0 \
  --use_pde --lam_pde 100.0 --use_bc --lam_bc 0.0 --lam_sup 1.0 \
  --use_energy --lam_energy 1.0 \
  --air-anchor 1e-1 \
  --spec-pad-type reflect --spec-pad-z 8 --spec-pad-y 8 --spec-pad-x 8 \
  --out_dir "model_param/heatsink_fno_pino_h" \
...
class HeatsinkH5:
    def __init__(self, path, preload_to_device: str = None):
        ...
        if 'sources' in self.f:
            ...
            self.q_mask = self.f['sources/q_mask'][:].astype(np.float32) if 'q_mask' in self.f[
                'sources'] else np.ones_like(self.q_vol, np.float32)
        else:
...
class MultiPairDataset(Dataset):
    """
    与你之前版本保持一致：每次采样 (t0, t1=t0+k)，k∈[1,max_skip]。
    预加载到 GPU（每个 H5 驻留一次）或 CPU 懒加载两条路径兼容。
    """
    ...
    def __init__(self, paths, max_skip=1, preload_to: str = None):
        ...
        for i, p in enumerate(self.paths):
            ...
            def q(v): return int(round(v / Q))  # 量化成整数键
    ...
    def __getitem__(self, idx):
        ...
        T0 = torch.from_numpy(H.T[t0]);
        T1 = torch.from_numpy(H.T[t1])
        theta0 = (T0 - T_BASE) / DT_BASE
        theta1 = (T1 - T_BASE) / DT_BASE
...
class MultiScaleSpectralBlock(nn.Module):
    """
    uFNO: 多分辨率频域分支的加权和（softmax 权重）。
    若 scale_count=1，则退化为单一 SpectralConv3d。
    """
    ...
    def forward(self, h):
        ...
        ws = torch.softmax(self.scale_logits, dim=0)  # (S,)
        y = 0.0
...
# =======================================================
# Bucketing：按尺寸分桶的 BatchSampler
# =======================================================
class ShapeBucketBatchSampler(torch.utils.data.Sampler):
    """
    将 dataset 样本按 (Nz,Ny,Nx) 分桶；每个 batch 从同一桶取，保证尺寸一致。
    - drop_last: True 时丢弃每个桶内最后一个不足 batch_size 的小批。
    - shuffle:   每个 epoch 内对桶和桶内样本都打乱。
    使用方法：DataLoader(..., batch_sampler=ShapeBucketBatchSampler(...))
    """

    def __init__(self, dataset: MultiPairDataset, batch_size: int, drop_last: bool = False,
        ...
        # 构建：shape -> [dataset_idx,...]
        self.shape2idxs = {}
        for ds_idx, (fi, _t0) in enumerate(self.ds.index):
            # _Nt, Nz, Ny, Nx = self.ds.meta[fi]
            # key = (Nz, Ny, Nx)
            # self.shape2idxs.setdefault(key, []).append(ds_idx)

            meta = self.ds.meta[fi]
            if len(meta) >= 7:
                _Nt, Nz, Ny, Nx, qdz, qdy, qdx = meta
                key = (Nz, Ny, Nx, qdz, qdy, qdx)
            else:
                _Nt, Nz, Ny, Nx = meta
                key = (Nz, Ny, Nx)
            self.shape2idxs.setdefault(key, []).append(ds_idx)
    ...
    def __iter__(self):
        ...
        for shape, idxs in self.shape2idxs.items():
            ...
            if (not self.drop_last) and (end < len(idxs)):
                full_batches.append(idxs[end:])  # 最后一个不足 batch_size 的也要
            per_bucket_batches.append(full_batches)

        # 2) 桶级洗牌 & 交织输出
        # 先打乱桶顺序，再轮转抽取每个桶的一个 batch，直到所有桶都耗尽
    ...
    def __len__(self):
        # 估计：所有桶可产生的 batch 数之和（与 drop_last 一致）
        total = 0
...
def train(args):
    ...
    if use_bucket and (not args.preload_gpu or args.batch > 1):
        # 注意：如果 batch=1，其实是否分桶都无所谓；这里也可照用 batch_sampler。
        batch_sampler = ShapeBucketBatchSampler(
            ds, batch_size=args.batch, drop_last=args.drop_last,
            shuffle=True, seed=args.seed
        )
        # 预加载到 GPU 时，仍然建议 num_workers=0（因为数据已在 GPU）
        if args.preload_gpu and (device.type == "cuda"):
            eff_workers = 0;
            eff_pin = False;
            eff_persist = False
        else:
            eff_workers = args.num_workers;
            eff_pin = args.pin_memory;
            eff_persist = (args.num_workers > 0)
        ...
        # 回退：原始 DataLoader（注意：不同形状时 batch>1 可能仍会报错）
        if args.preload_gpu and (device.type == "cuda"):
            eff_workers = 0;
            eff_pin = False;
            eff_persist = False
    ...
    for ep in range(start_ep, args.epochs + 1):
        ...
        for batch in dl:
            ...
            if dxh_dyh_dzh.dim() == 2 and dxh_dyh_dzh.shape[0] > 1:
                if not torch.allclose(dxh_dyh_dzh, dxh_dyh_dzh[0:1].repeat(dxh_dyh_dzh.shape[0], 1)):
                    raise ValueError("Batch 内存在不同的网格间距，请设置 --batch 1 或开启分桶并确保同形状采样。")
            ...
            # ===== 多步 / RK2 时间推进 =====
            # 用 multi-steps 做短滚动，detach-every 控制是否隔步截断图
            Bsz = x.size(0)
            theta_curr = x[:, 0:1]  # 当前 θ
            ...
            for step in range(args.multi_steps):
                ...
                # 监督：仅拿 batch 中提供的 y 作为 “一步” 目标（与你现有一致）
                ...
                if args.use_pde:
                    if use_fv:
                        res_p = pde_residual_fv(theta_next, theta_curr, dtau, Ms, Mi,
                                                (cond["h_init"].to(device) * Mi * L_over_k) * 0 +  # dummy,不影响
                                                (0.0),
                ...
                if args.use_bc:
                    ...
                    h_data = h_truth_t0 * has_h + (1.0 - has_h) * cond["h_init"].to(device,
                                                                                    non_blocking=True).float().repeat(
                        Bsz, 1, 1, 1, 1)
                    h_data = h_data * Mi
                ...
                # 规范化的 PDE/BC 项（与你版本一致）
                scale_p_ini = _op_norm_factor(dxh_dyh_dzh, dtau)
                res_p_n = (res_p.double() / (scale_p_ini + 1e-12)).float() if args.use_pde else torch.tensor(0.0,
                                                                                                             device=device)
                pde_n = ((res_p_n ** 2) * mask_int).sum() / (mask_int.sum() + 1e-8) if args.use_pde else torch.tensor(
                    0.0, device=device)
                c_pde = 0.5
                ...
                if args.use_energy:
                    ...
                    T_true = y * DT_BASE + T_BASE  # 以 batch 内的 y 作参照
                    Q_pred = (Bi_eff.double() * A.double() * (T_pred.double() - theta_inf_gridK.double())).sum(
                        dim=(1, 2, 3, 4))
                    Q_ref = (Bi_eff.double() * A.double() * (T_true.double() - theta_inf_gridK.double())).sum(
                        dim=(1, 2, 3, 4))
                ...
                # 下一步
                theta_curr = theta_next.detach() if (
                            args.detach_every > 0 and ((step + 1) % args.detach_every == 0)) else theta_next
        ...
        print(f"[ep {ep:03d}]")
        print(
            f"  loss_terms: (lam_sup*sup)={m_sup_term / n_batches:.4e}  "
            f"pde_term={m_pde_term / n_batches:.4e}  bc_term={m_bc_term / n_batches:.4e}  "
            f"{'energy_term=' + format((m_energy / n_batches), '.4e') if args.use_energy else ''}  "
            f"air_terms={m_air_term / n_batches:.4e}  total={m_total_det / n_batches:.4e}"
        )
        print(
...
def parse_args():
    ...
    ap.add_argument("--mz", type=int, default=12);
    ap.add_argument("--my", type=int, default=12);
    ap.add_argument("--mx", type=int, default=12)

    ap.add_argument("--lam_sup", type=float, default=1.0)
    ap.add_argument("--use_pde", action="store_true");
    ap.add_argument("--lam_pde", type=float, default=1e-2)
    ap.add_argument("--use_bc", action="store_true");
    ap.add_argument("--lam_bc", type=float, default=1e-2)
    ...
    ap.add_argument("--max_skip", type=int, default=1, help="每个样本内部随机跨度上限K（k∈[1,K]）")
    ...
    # uFNO
    ap.add_argument("--ufno-scales", type=int, default=3, help="=1 等价 FNO；=3 为多尺度(m, m/2, m/4)")

    # Bucketing
    ap.add_argument("--bucket_by_shape", "--bucket-by-shape", dest="bucket_by_shape", action="store_true", default=True,
                    help="开启后按 (Nz,Ny,Nx) 分桶采样，保证 batch 内尺寸一致")
    ap.add_argument("--drop-last", action="store_true", help="每个桶内最后不足 batch_size 的小批丢弃")
...
if __name__ == "__main__":
    args = parse_args()
    # 小提示：预加载+多步可能吃显存
    if args.preload_gpu and args.multi_steps > 1:
