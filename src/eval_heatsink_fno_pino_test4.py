...
"""
与新的训练脚本对齐的评估版：
- 无量纲温度：theta = (T - T_BASE)/DT_BASE
- 源项：S_nd = q_vol * L^2 / (alpha * rho * cp * DT_BASE)
- dtau = dt * alpha / L^2
- BC/PDE：Bi_eff = h_data * (L/k)
- FNO 使用 3 通道输入 [theta_in, Ms, S_nd]，并注入 9 维全局 ctx（含 log(alpha)）
- 若 ckpt 含 hhead：仅用于导出/统计（不参与 Bi_eff）；hhead in_c=7，ctx_dim=9
- Kelvin 重构使用 T_BASE/DT_BASE
"""
...
'''
python src/eval_heatsink_fno_pino_test3.py \
...
python src/eval_heatsink_fno_pino_test4.py \
  --data dat/gridized_vtk_padded_with_scales_h.h5 \
  --ckpt dat/runs/heatsink_fno_pino_learnh/ckpt_ep200.pt \
  --out  dat/eval_heatsink_tf.h5 \
  --teacher_forced \
  --save_h_uniform_energy_balance_all \
  --h_smooth_dt_thresh 0.5 \
  --print_h_stats \
...
def surface_flux_and_h_from_grad(theta, nsurf5, Ms5, Mi5, dz_hat, dy_hat, dx_hat,
    ...
    从无量纲温度场 theta (Nz,Ny,Nx) 计算：
      - 表面法向热流 q_n [W/m^2]
      - 局部换热系数 h [W/(m^2*K)]
    使用固体侧单边梯度（与训练一致的一阶版本）。
    BASE 标度：q_n = -(k*DT_BASE/L) * dθ/dn_hat,  T_s = T_BASE + θ*DT_BASE
    """
    device = theta.device
    ...
    # 物理热流：q_n = -(k * DT_BASE / L) * dθ/dn_hat
    qn = -(k * DT_BASE / L) * dth_dn_hat                      # (1,1,Nz,Ny,Nx)

    # 表面温度与 T_inf
    T_surf = theta * DT_BASE + T_BASE                         # (Nz,Ny,Nx)
...
class HeatsinkH5:
    def __init__(self, path):
        ...
        bc = self.f['bc/robin']
        self.h_init = bc['h_init'][:].astype(np.float32)   # 逐体素初值 h
        ...
        if 'sources' in self.f:
            sg = self.f['sources']
            self.q_vol = (sg['q_vol'][:] if 'q_vol' in sg else np.zeros((self.Nz,self.Ny,self.Nx), np.float32)).astype(np.float32)
            self.q_mask= (sg['q_mask'][:] if 'q_mask' in sg else np.ones_like(self.q_vol, np.float32)).astype(np.float32)
        else:
        ...
        self.T_amb = float(cst['T_amb'][()]) if 'T_amb' in cst else T_BASE  # 仅供参考
        ...
        # scales：alpha 优先取文件；否则用 k/(rho*cp)
        sc  = self.f['scales']
        file_alpha = float(sc['alpha'][()]) if 'alpha' in sc else None
        self.L      = float(sc['L'][()])       # 仍用于 θ 的无量纲化、梯度换算
        self.dTref  = float(sc['dT_ref'][()]) if 'dT_ref' in sc else DT_BASE  # 仅参考输出
        self.qref   = float(sc['q_ref'][()]) if 'q_ref' in sc else None       # 仅参考输出
        ...
        # 无量纲网格间距（与训练一致传 (dz,dy,dx)）
        self.dx_hat = self.dx/self.L; self.dy_hat = self.dy/self.L; self.dz_hat = self.dz/self.L

        # ctx_glb（9 维）：log Lz, log Ly, log Lx, g_hat(z,y,x), log|g|, mean(theta_inf_hat), log(alpha)
...
# —— 与训练一致的 HHead（仅用于导出/统计；不参与 Bi_eff） ——
...
def evaluate(args):
    ...
    S4  = torch.from_numpy(H.S_nd[None,...]).float()   # 新：使用 S_nd
    ...
    L_over_k = torch.tensor([H.L_over_k], dtype=torch.float32)

    # --- 构建 theta_inf（无量纲，BASE）
    theta_inf5 = ((torch.from_numpy(H.T_inf).float() - float(T_BASE)) / float(DT_BASE)).view(1,1,Nz,Ny,Nx)

    # --- build model
    ckpt = torch.load(args.ckpt, map_location='cpu')
    a = ckpt["args"]
    model = FNO3D(in_c=3, width=a["width"], modes=(a["mz"],a["my"],a["mx"]), layers=a["layers"], context_dim=9).to(device)
    model.load_state_dict(ckpt["model"]); model.eval()
    learn_h = ("hhead" in ckpt)
    if learn_h:
        hhead = HHead(in_c=7, width=a.get("h_width",16), layers=a.get("h_layers",2), ctx_dim=9).to(device)
        hhead.load_state_dict(ckpt["hhead"]); hhead.eval()
        ctx_glb = torch.from_numpy(H.ctx_glb).float().unsqueeze(0)  # (1,9)
    else:
        hhead = None
        ctx_glb = torch.from_numpy(H.ctx_glb).float().unsqueeze(0)  # 仍构造，供 FNO 使用
    ...
    # --- 初值
    theta_prev = torch.from_numpy((H.T[0]-T_BASE)/DT_BASE).float()
    ...
    for i in range(Nt-1):
        theta_true_t     = torch.from_numpy((H.T[i  ]-T_BASE)/DT_BASE).float()
        theta_next_true  = torch.from_numpy((H.T[i+1]-T_BASE)/DT_BASE).float()
        ...
        # 前向：r = dθ/dτ
        x = torch.stack([theta_in, Ms4[0], S4[0]], dim=0).unsqueeze(0)  # in_c=3
        r = model(x, ctx_glb)
        ...
        # ====== 构造“数据驱动”的逐体素 h（与训练一致）：h_truth[i] 优先，否则 h_init ======
        if H.h_truth is not None:
            h_data = torch.from_numpy(H.h_truth[i]).float().view(1,1,Nz,Ny,Nx)
        else:
            h_data = torch.from_numpy(H.h_init).float().view(1,1,Nz,Ny,Nx)
        h_data = h_data * Mi5  # 仅界面
        Bi_eff = h_data * L_over_k.view(1,1,1,1,1)

        # PDE 残差（FV，与训练一致）
        res_p = pde_residual_fv(
            y_pred[None,None,...], theta_in[None,None,...], dtau, Ms5, Mi5, Bi_eff, dxh_dyh_dzh, S5, theta_inf=theta_inf5
        )
        pde_rms = torch.sqrt( ((res_p**2)*Ms5).sum() / (Ms5.sum()+1e-8) ).item()
        per_pair["pde_rms"].append(pde_rms)

        # BC 残差（与训练一致）
        res_b = bc_robin_residual(
            y_pred[None,None,...], Bi_eff, nsurf5, dxh_dyh_dzh, Ms=Ms5, Mi=Mi5, theta_inf=theta_inf5
        )
        bc_rms = torch.sqrt( ((res_b**2)*Mi5).sum() / (Mi5.sum()+1e-8) ).item()
        per_pair["bc_rms"].append(bc_rms)

        # ====== 小网络 h（仅导出/统计；不影响 Bi_eff） ======
        if learn_h:
            feats = torch.cat([
                theta_in[None,None,...],     # θ0
                S5, Mi5, Ms5,
                nsurf5[:,0:1], nsurf5[:,1:2], nsurf5[:,2:3],
            ], dim=1)  # in_c=7
            h_pred_field = hhead(feats, ctx_vec=ctx_glb) * Mi5  # (1,1,Z,Y,X)

            if args.print_h_stats:
                h_np = h_pred_field[0,0].detach().cpu().numpy().astype(np.float32)
                mask = (H.Mi > 0)
                if mask.sum() > 0:
                    vals = h_np[mask]
                    h_min  = float(vals.min()); h_max = float(vals.max()); h_mean = float(vals.mean())
                    A_np = A
                    h_aw   = float((vals * A_np[mask]).sum() / (A_np[mask].sum() + 1e-8))
                    h_stats.append((i+1, h_min, h_max, h_mean, h_aw))
            if h_pred_seq is not None:
                h_pred_seq.append(h_pred_field[0,0].detach().cpu().numpy().astype(np.float32))
        ...
        if save_all_pred or need_last_pred:
            qn_pred, h_pred_grad, Ts_pred = surface_flux_and_h_from_grad(
                ...
                k=H.k, L=H.L, T_inf=H.T_inf
            )
            # 用“数据 h”做一致性校核（物理：q = h * ΔT）
            T_inf_t = torch.from_numpy(H.T_inf).to(Ts_pred)
            qn_from_h = h_data[0,0].to(Ts_pred.dtype).to(Ts_pred.device) * (Ts_pred - T_inf_t)
            Mi_t = torch.from_numpy(H.Mi).to(qn_pred)
            diff_q = ((qn_pred - qn_from_h) * (Mi_t > 0.5)).abs()
        ...
        if save_all_truth or need_last_truth:
            ...
            # 一致性：仍用“数据 h”
            if H.h_truth is not None:
                h_truth_here = torch.from_numpy(H.h_truth[i+1]).float().to(qn_true).view(Nz,Ny,Nx)
            else:
                h_truth_here = torch.from_numpy(H.h_init).float().to(qn_true).view(Nz,Ny,Nx)
            T_inf_t = torch.from_numpy(H.T_inf).to(Ts_true)
            qn_from_h_true = h_truth_here * (Ts_true - T_inf_t)
            Mi_t = torch.from_numpy(H.Mi).to(qn_true)
            diff_q_true = ((qn_true - qn_from_h_true) * (Mi_t > 0.5)).abs()
        ...
        if save_all_h_eb:
            ...
            Ts_np = (theta_next_true.numpy().astype(np.float32) * DT_BASE + T_BASE)
            ...
            if denom_area <= 0:
                ...
                Q_from_uniform = h_eb * denom_area
    ...
    summary = {k: {
        ...
        "std": float(np.std(v)) if len(v)>0 else 0.0,
        "p50": float(np.percentile(v,50)) if len(v)>0 else 0.0,
        "p90": float(np.percentile(v,90)) if len(v)>0 else 0.0
    } for k,v in per_pair.items()}

    # ---- learn_h 的 h 场（最后一帧预测；仅导出）
    h_artifacts = None
    if learn_h and args.save_h:
        feats_last = torch.cat([
            theta_last_pred[None,None,...],  # θ0 取最后一步的预测（或输入）
            S5, Mi5, Ms5,
            nsurf5[:,0:1], nsurf5[:,1:2], nsurf5[:,2:3],
        ], dim=1)
        h_pred_last = (hhead(feats_last, ctx_vec=ctx_glb) * Mi5)[0,0].cpu().numpy().astype(np.float32)
        h_artifacts = {
            "h_pred_Wm2K":          h_pred_last,
    ...
    with h5py.File(args.out, "w") as fo:
        ...
        if args.save_h_from_grad and 'qn_last' in locals():
        ...
        if args.save_h_from_grad_truth_uniform_all and len(qn_list_true) > 0:
            ...
            h_uniform = np.divide(QA, dTA, out=np.zeros_like(QA, dtype=np.float32), where=dTA>0).astype(np.float32)
        ...
        if args.save_h_uniform_energy_balance_all and len(h_eb_seq) > 0:
            h_eb_seq_arr = np.asarray(h_eb_seq, dtype=np.float32)
            h_eb_field_seq_arr = np.stack(h_eb_field_seq, axis=0)
        ...
        fo.attrs["note"]  = (
            ...
            "Kelvin reconstruction uses T_BASE/DT_BASE."
        )
