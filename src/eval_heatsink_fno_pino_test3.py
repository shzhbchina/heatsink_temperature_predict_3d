...
"""
用法与旧版一致；关键变化：
- FNO 输入通道=3（[theta_in, Ms, S]），不再把 dtau 当作通道；
- BC/PDE 一律用数据 h（h_truth 若有，否则 h_init），并使用 L_char/k；
- 如果 ckpt 内含 hhead，仅用于保存/统计，不参与 Bi_eff。
"""
...
class HeatsinkH5:
    def __init__(self, path):
        ...
        # —— 关键：L_char 及其缩尺 ——（与训练一致）
        self.L_char = abs(self.g_hat[0])*self.Lz_abs + abs(self.g_hat[1])*self.Ly_abs + abs(self.g_hat[2])*self.Lx_abs
        if self.L_char < 1e-12:
            self.L_char = max(self.Lz_abs, self.Ly_abs, self.Lx_abs)
        self.Lchar_over_k = self.L_char / self.k
        self.Bi_L = (self.h_init*self.L/self.k).astype(np.float32)  # 仅参考，不再用于 BC

        # HHead“上下文参数”（与训练一致）
        self.ctx_vec = np.array([
...
class HHead(nn.Module):
    def __init__(self, in_c=8, width=32, layers=2, out_feat=64, mlp_hidden=64,
                 ...
                 h_prior=10.0, beta=2.0, use_ctx_film=True, ctx_dim=7):
        super().__init__()
...
def evaluate(args):
    ...
    Lchar_over_k = torch.tensor([H.Lchar_over_k], dtype=torch.float32)

    # --- 构建 theta_inf（无量纲）
    theta_inf5 = ((torch.from_numpy(H.T_inf).float() - float(H.T_amb)) / float(H.dTref)).view(1,1,Nz,Ny,Nx)

    # --- build model
    ckpt = torch.load(args.ckpt, map_location='cpu')
    a = ckpt["args"]
    model = FNO3D(in_c=3, width=a["width"], modes=(a["mz"],a["my"],a["mx"]), layers=a["layers"]).to(device)
    model.load_state_dict(ckpt["model"]); model.eval()
    learn_h = ("hhead" in ckpt)
    if learn_h:
        hhead = HHead(in_c=8, width=a.get("h_width",32), layers=a.get("h_layers",2)).to(device)
        hhead.load_state_dict(ckpt["hhead"]); hhead.eval()
        ctx_vec = torch.from_numpy(H.ctx_vec).float().unsqueeze(0)  # (1,7)
    else:
        hhead = None
        ctx_vec = None
    ...
    for i in range(Nt-1):
        ...
        Bi_eff = h_data * Lchar_over_k.view(1,1,1,1,1)
        ...
        if learn_h:
            ...
            h_pred_field = hhead(feats, ctx_vec=ctx_vec) * Mi5  # (1,1,Z,Y,X)
    ...
    if learn_h and args.save_h:
        feats_last = torch.cat([
            theta_last_pred[None,None,...],  # θ0 取最后一步输入或预测均可
            S5, Mi5, Ms5,
            nsurf5[:,0:1], nsurf5[:,1:2], nsurf5[:,2:3],
            theta_inf5
        ], dim=1)
        h_pred_last = (hhead(feats_last, ctx_vec=ctx_vec) * Mi5)[0,0].cpu().numpy().astype(np.float32)
        h_artifacts = {
