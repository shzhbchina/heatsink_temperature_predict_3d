# -*- coding: utf-8 -*-
"""
v16 (FULLSEQ 多步监督 & 广播修复版)  + 任选两帧一步跨越训练
训练4层网络
...
本次额外修改（重要）：
- PDE/BC 仍然使用 bc/robin/h_init（通过 Bi_eff）；
- 能量项 energy loss 改为优先使用
  /h_uniform_from_sources_all/h_uniform_scalar_Wm2K 这一维数组给出的每个时间点的平均换热系数，
  即用“标量 h(t)”构造 Bi_eff_energy，只影响 energy loss；如果数据中不存在该数组，则回退到旧逻辑。

python src/train_fno_pino_h_16_cuda.py \
...
class HeatsinkH5:
    def __init__(self, path, preload_to_device: str = None):
        ...
        # ========= h 真值(场) + 每个时间点的标量平均换热系数 =========
        self.h_truth = None           # (Nt, Nz, Ny, Nx) 可选
        self.h_scalar_seq = None      # (Nt,)  每个时间点的平均 h（W/m^2/K）
        ...
        if 'h_uniform_from_sources_all' in self.f:
            ...
            if 'h_uniform_scalar_Wm2K' in g:
                self.h_scalar_seq = g['h_uniform_scalar_Wm2K'][:].astype(np.float32).reshape(-1)
        ...
        if (self.h_truth is None) and ('h_from_grad_truth_uniform_all' in self.f):
            ...
            if (self.h_scalar_seq is None) and ('h_uniform_scalar_Wm2K' in g2):
                self.h_scalar_seq = g2['h_uniform_scalar_Wm2K'][:].astype(np.float32).reshape(-1)

        if (self.h_scalar_seq is not None) and (self.h_scalar_seq.shape[0] != self.Nt):
            print(f"[warn] h_uniform_scalar_Wm2K len={self.h_scalar_seq.shape[0]} != Nt={self.Nt} in file {path}")
        ...
        if preload_to_device is not None:
            ...
            self.hscalar_t = torch.from_numpy(self.h_scalar_seq).to(device=dev) if self.h_scalar_seq is not None else None
...
class MultiPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if getattr(H, "on_device", None) == "cuda":
            ...
            # 新增：标量 h(t0), h(t1)，用于能量项
            if getattr(H, "hscalar_t", None) is not None:
                cond["h_scalar_t0"] = H.hscalar_t[t0].view(1)  # (1,)
                cond["h_scalar_t1"] = H.hscalar_t[t1].view(1)  # (1,)
        ...
        if H.h_scalar_seq is not None:
            cond["h_scalar_t0"] = torch.tensor([H.h_scalar_seq[t0]], dtype=torch.float32)
            cond["h_scalar_t1"] = torch.tensor([H.h_scalar_seq[t1]], dtype=torch.float32)
...
class AnyTwoFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if getattr(H, "on_device", None) == "cuda":
            ...
            if getattr(H, "hscalar_t", None) is not None:
                cond["h_scalar_t0"] = H.hscalar_t[ts].view(1)
                cond["h_scalar_t1"] = H.hscalar_t[te].view(1)
        ...
        if H.h_scalar_seq is not None:
            cond["h_scalar_t0"] = torch.tensor([H.h_scalar_seq[ts]], dtype=torch.float32)
            cond["h_scalar_t1"] = torch.tensor([H.h_scalar_seq[te]], dtype=torch.float32)
...
class FullSeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if getattr(H, "on_device", None) == "cuda":
            ...
            if getattr(H, "hscalar_t", None) is not None:
                cond["h_scalar_seq"] = H.hscalar_t  # (Nt,)
        ...
        if H.h_scalar_seq is not None:
            cond["h_scalar_seq"] = torch.from_numpy(H.h_scalar_seq).float()  # (Nt,)
        return theta_seq, cond
            ...
            if args.fullseq:
                ...
                for s in range(0, Nt - 1, win):
                    ...
                    for t in range(s, e):
                        ...
                        if args.use_energy:
                            # 优先使用标量 h(t+1) 构造 Bi_eff_energy；若无则退回 Bi_eff
                            h_scalar_seq = cond.get("h_scalar_seq", None)
                            if h_scalar_seq is not None:
                                h_scalar_seq = h_scalar_seq.to(device, non_blocking=True).float()
                                if h_scalar_seq.dim() == 1:
                                    idx = min(t + 1, h_scalar_seq.numel() - 1)
                                    h_t = h_scalar_seq[idx].view(1).repeat(B)  # (B,)
                                else:
                                    idx = min(t + 1, h_scalar_seq.size(1) - 1)
                                    h_t = h_scalar_seq[:, idx]                # (B,)
                                h_t = h_t.view(B, 1, 1, 1, 1)
                                Bi_eff_energy = (h_t * L_over_k) * Mi
                            else:
                                Bi_eff_energy = Bi_eff

                            T_pred = theta_next * DT_BASE + T_BASE
                            T_true = theta_true_next * DT_BASE + T_BASE
                            Q_pred = (Bi_eff_energy.double() * A.double()
                                      * (T_pred.double() - TinfK.double())).sum(dim=(1, 2, 3, 4))
                            Q_ref  = (Bi_eff_energy.double() * A.double()
                                      * (T_true.double() - TinfK.double())).sum(dim=(1, 2, 3, 4))
                ...
                if args.use_energy:
                    ...
                    h_scalar_t1 = cond.get("h_scalar_t1", None)
                    if h_scalar_t1 is not None:
                        h_s = h_scalar_t1.to(device, non_blocking=True).float()
                        h_s = h_s.view(h_s.size(0), -1)
                        h_s = h_s[:, 0].view(-1, 1, 1, 1, 1)
                        Bi_eff_energy = (h_s * L_over_k) * Mi
                    else:
                        Bi_eff_energy = Bi_eff
