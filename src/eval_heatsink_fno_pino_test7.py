...
class FNO3D(nn.Module):
            ...
            if self.ufno:
                self.specs.append(
                    MultiResSpectral3DBlock(
                        ...
                        pool_type=self.ufno_pool, up_mode=self.ufno_upmode
                    )
                )
            else:
    ...
    def forward(self, x, ctx: torch.Tensor = None):
        ...
        for i, (spec_block, w, gn) in enumerate(zip(self.specs, self.ws, self.norms)):
            y = spec_block(h) + w(h)
            if self.use_local:
                y = y + self.locals[i](h)
            y = F.gelu(y)
...
def load_ckpt_for_eval(ckpt_path, device='cpu'):
    ...
    model = FNO3D(
        ...
        ufno=ufno_enable, ufno_scales=ufno_scales,
        ufno_pool=ufno_pool, ufno_upmode=ufno_upmode
    ).to(device)
...
def evaluate_modes(H: HeatsinkH5, model: FNO3D, hhead: HHead, rk2: bool,
    ...
    if do_rollout:
        ...
        if rollout_steps is None or int(rollout_steps) < 0:
            ...
            rollout_steps_eff = int(max(0, min(int(rollout_steps), (Nt - 1) - int(rollout_from))))
