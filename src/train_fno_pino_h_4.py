...
'''
python /workspace/src/train_fno_pino_h_4.py \
...
class HeatsinkH5:
    def __init__(self, path):
        ...
        self.L      = float(sc['L'][()])     # 用于 θ 与 (x/L) 的无量纲化
...
class PairDataset(torch.utils.data.Dataset):
    def __init__(self, H: HeatsinkH5):
        ...
        self.dxh, self.dyh, self.dzh = H.dz_hat, H.dy_hat, H.dx_hat  # 注意：下游函数按 (dz,dy,dx) 取值
        self.h_truth = torch.from_numpy(H.h_truth) if (H.h_truth is not None) else None

        # 全局参数（不进 voxel 通道）
        self.ctx_glb = torch.from_numpy(H.ctx_glb)        # (8,)
    ...
    def __getitem__(self, idx):
        ...
        # FNO 输入：仍为 3 通道（不把 g、theta_inf 当体素通道）
...
class HHead(nn.Module):
    def __init__(self, in_c=7, width=32, layers=2, out_feat=64, mlp_hidden=64,
                 ...
                 h_prior=10.0, beta=2.0, use_ctx_film=True, ctx_dim=8):
        """
        in_c=7 组成：[θ0, S, Mi(作为mask_idx=2), Ms, nz, ny, nx]
        不把 theta_inf 当体素通道；其信息由 ctx 全局向量（含 mean(theta_inf_hat)）调制。
        """
        super().__init__()
...
def build_env_channels(cond, H, x):
    ...
    theta_inf = theta_inf.repeat(B,1,1,1,1)    # (B,1,Z,Y,X) 仅用于物理损失
    return theta_inf
...
def train(args):
    ...
    print(f"[info] scales: alpha={H.alpha:.3e}, L_ref={H.L:.3e}, dTref={H.dTref:.3e}, qref={H.qref:.3e}, L/k={H.L_over_k:.4e}")
    ...
    for ep in range(start_ep, args.epochs+1):
        ...
        for x, y, cond in dl:
            ...
            # 参数：L/k 与 全局 context（g + 环境温度参数）
            L_over_k = cond["L_over_k"].to(device).view(1,1,1,1,1)
            ctx_glb = cond["ctx_glb"].to(device).repeat(x.size(0),1)  # (B,8)
            ...
            if "h_truth_t0" in cond and cond["h_truth_t0"] is not None:
                ...
                h_data = cond["h_init"].to(device).float().repeat(Bsz,1,1,1,1)
            h_data = h_data * Mi
            Bi_eff = h_data * L_over_k   # Bi = h * L / k

            # ====== hhead：保留并监督，但不参与 Bi_eff（除非你后续想开启）======
