...
"""
v11 (在 v10 基础上默认启用 UFNO)

新增点：
1) (A) 逐层 FiLM：FNO3D 每一层用 ctx 生成 (gamma,beta)，对层内特征做 (1+gamma)*y + beta。（保持 v10）
2) (C) 频域门控：SpectralConv3d 对频域支路输出施加可学习缩放 scale=1+tanh(gate)，初始为 1。（保持 v10）
3) (U) UFNO 多分辨率：每层并行 1×/2×/4× 频域分支，上采样回原分辨率后用可学习 softmax 权重融合（默认开启）。

其余功能保持原样：Δθ 预测、FV/strong PDE 残差、Robin BC、能量约束、频域 padding、非量纲化等。
...
python src/train_fno_pino_h_11_cuda.py \
...
python src/train_fno_pino_h_11_cuda.py \
  ...
  --spec-pad-type reflect --spec-pad-z 8 --spec-pad-y 8 --spec-pad-x 8 \
  --max_skip 1 --out_dir "model_param/heatsink_fno_pino_learnh" \
  --ckpt "model_param/heatsink_fno_pino_learnh/ckpt_ep100.pt" \
  --device cuda:0 --num-workers 0 --pin-memory \
  --preload-gpu \
  --resume
...
class SpectralConv3d(nn.Module):
    def __init__(self, in_c, out_c, modes_z, modes_y, modes_x,
        ...
        # (C) 频域门控参数：对频域支路输出做缩放，初始化为 1.0（tanh(0)=0 → 1+0）
    ...
    def forward(self, x):
        ...
        # ★ 频域门控：对 spectral 分支输出做可学习缩放（起始=1.0，范围约 (0,2)）
...
class MultiResSpectral3DBlock(nn.Module):
    ...
    def __init__(self, width: int, modes: tuple, pad_type: str,
        ...
        super().__init__()
        assert num_scales >= 1 and num_scales <= 4
        self.width = width
        ...
        nn.init.constant_(self.scale_logits, 0.0)
    ...
    def _down(self, x, s):
        ...
        if self.pool_type == 'max':
            return F.max_pool3d(x, kernel_size=k, stride=k, padding=0)
        else:
            return F.avg_pool3d(x, kernel_size=k, stride=k, padding=0)

    def _up(self, x, size_hwz):
        return F.interpolate(x, size=size_hwz, mode=self.up_mode, align_corners=False)
...
class FNO3D(nn.Module):
    def __init__(self, in_c=3, width=24, modes=(12, 12, 12), layers=4,
        ...
        self.ufno = bool(ufno)
        self.ufno_scales = int(ufno_scales)
        self.ufno_pool = ufno_pool
        self.ufno_upmode = ufno_upmode
        ...
        # 主干层：UFNO 多分辨率或单尺度谱卷积
        self.specs = nn.ModuleList()
        for _ in range(layers):
            if self.ufno:
                self.specs.append(
                    MultiResSpectral3DBlock(
                        width=width, modes=(mz, my, mx),
                        pad_type=self.spec_pad_type, spec_pad=(pz, py, px),
                        num_scales=self.ufno_scales,
                        pool_type=self.ufno_pool,
                        up_mode=self.ufno_upmode
                    )
                )
            else:
                self.specs.append(
        ...
        # (A) 逐层 FiLM：若有 ctx，则每层生成 (gamma,beta)
        ...
        if self.context_dim > 0:
            for _ in range(layers):
                ...
                nn.init.zeros_(lin.weight); nn.init.zeros_(lin.bias)  # 初始为恒等（gamma=0,beta=0）
    ...
    def forward(self, x, ctx: torch.Tensor = None):
        ...
        for i, (spec_or_block, w, gn) in enumerate(zip(self.specs, self.ws, self.norms)):
            y = spec_or_block(h) + w(h)
            if self.use_local:
                y = y + self.locals[i](h)
            y = F.gelu(y)
            ...
            # ★ (A) 逐层 FiLM：对 y 做 (1+gamma)*y + beta
...
def train(args):
    ...
    # 模型（context_dim=10：9维 ctx_glb + 1维 log(dtau)，UFNO 默认开启）
    model = FNO3D(
        ...
        ufno_pool=args.ufno_pool,
        ufno_upmode=args.ufno_upmode
    ).to(device)
...
def parse_args():
    ...
    # === UFNO 多分辨率（默认开启） ===
    ap.add_argument("--ufno", dest="ufno", action="store_true", default=True,
                    help="启用多分辨率 FNO（UFNO），默认开启")
    ap.add_argument("--no-ufno", dest="ufno", action="store_false",
                    help="禁用 UFNO")
    ap.add_argument("--ufno-scales", type=int, default=3,
                    help="多分辨率分支数（建议 2~4，默认3：1x/2x/4x）")
    ap.add_argument("--ufno-pool", type=str, choices=["avg", "max"], default="avg",
                    help="下采样方式：avg 或 max")
    ap.add_argument("--ufno-upmode", type=str, choices=["trilinear", "nearest"], default="trilinear",
                    help="上采样插值模式")
