...
新增点：
1) (A) 逐层 FiLM：FNO3D 每一层用 ctx 生成 (gamma,beta)，对层内特征做 (1+gamma)*y + beta。
2) (C) 频域门控：SpectralConv3d 对频域支路输出施加可学习缩放 scale=1+tanh(gate)，初始为 1。
...
python src/train_fno_pino_h_10_cuda.py \
  --data_glob "dat/*.h5" \
  --epochs 200 --batch 1 --lr 1e-3 \
...
class SpectralConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # ★ (C) 频域门控：对 spectral 分支输出做可学习缩放（起始=1.0，范围约 (0,2)）
        scale = 1.0 + torch.tanh(self.gate)   # 形状 (1,out_c,1,1,1)，广播到 (B,out_c,Z,Y,X)
        y = y * scale
        return y
...
def train(args):
    ...
    # 模型（context_dim=10：9维 ctx_glb + 1维 log(dtau)）
    model = FNO3D(
